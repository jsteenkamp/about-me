
## Example Projects

Some examples of applications and components I've built.
### Deep Search

[Building a Production-Ready Deep Search Application: Architecture, Evals, and Telemetry](https://www.linkedin.com/pulse/building-production-ready-deep-search-application-evals-steenkamp-dbsyc/)

![Deep Search Sources](attachments/resume-deep-search-1.jpeg)

**Deep Search Sources** — Expandable showing sources scraped for detailed information

![Deep Search Research](attachments/resume-deep-search-2.jpeg)

**Deep Search Research** — Expandable showing LLM response with planned actions

![Deep Search Answer](attachments/resume-deep-search-3.jpeg)

**Deep Search Answer** — Final answer with links for reference sources

![Deep Search Local Evals](attachments/resume-deep-search-4.jpeg)

**Deep Search Local Evals** — Local evals run on your development server and make iteration quick and easy.

![Deep Search Engineering Observability](attachments/resume-deep-search-5.jpeg)

 **Deep Search Engineering Observability** — Platform provides continuous observability with traces, evals and provides prompt management and metrics for continual improvement.

### Darwinium Frontend

Views and components built for Darwinium Aphex frontend application

![Interactive Network Graphs](attachments/resume-darwinium-1.jpeg)

Interactive Network Graphs with custom nodes, grouping and filtering

![Dashboard Cards](attachments/resume-darwinium-2.jpeg)

Dashboard Cards with configurable components including charts and data grids

![Dashboards with configurable tabs](attachments/resume-darwinium-3.jpeg)

Dashboards with user configurable tabs, resizable panels, and layout

### Mersen R-TOOLS MAXX

[R-TOOLS MAXX Air and Liquid Cooled Heat Sink Selector Tool](https://www.mersen.com/en/resources/r-tools-maxx-air-and-liquid-cooled-heat-sink-selector-tool) is a online tool for simulating and selecting air or liquid-cooled heat sinks.

AWS tech stack: Amplify (React), Cognito, S3, DynamoDB, Lambda


![Projects List](attachments/resume-mersen-r-tools-maxx-1.png)

Projects List

![Thermal simulation of AquaFlow pipes](attachments/resume-mersen-r-tools-maxx-2.png)

Thermal simulation of AquaFlow pipes

![AquaFlow pipe layout](attachments/resume-mersen-r-tools-maxx-3.png)

AquaFlow pipe layout

![AquaFlow heat source layout](attachments/resume-mersen-r-tools-maxx-4.png)

AquaFlow heat source layout


![Mechanically Bonded Fin Selector](attachments/resume-mersen-r-tools-maxx-5%201.png)

Mechanically Bonded Fin Selector

![Mechanically Bonded Fin thermal simulation](attachments/resume-mersen-r-tools-maxx-6%201.png)

Mechanically Bonded Fin thermal simulation
